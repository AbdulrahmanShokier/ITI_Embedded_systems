...
u8 u8_ArrayIndex = 0;
u32 L_u32_Entered_ID;
u8 L_u8_Entered_Age;
u8 L_u8_Entered_Gender;
u8 L_u8_Entered_Gender_Check;
...
void AdmOpt1_void_Add_new_Patient(Patient* PointerToPatientsData)
{
	...
	else
	{
		...
		fflush(stdin);
		L_u8_Entered_Gender_Check=0;
		while(L_u8_Entered_Gender_Check==0)
		{
		printf("Enter the patient Gender\n");
		scanf("%c",&L_u8_Entered_Gender);
		fflush(stdin);
		if(L_u8_Entered_Gender== 'm' ||L_u8_Entered_Gender== 'M' ||
		   L_u8_Entered_Gender== 'F' ||L_u8_Entered_Gender== 'f'  )
		{
			L_u8_Entered_Gender_Check=1;
		}
		else
		{
			printf("The entered Gender is wrong\n");

		}
		}
		PointerToPatientsData[u8_ArrayIndex].L_u32_ID=L_u32_Entered_ID;
...
void AdmOpt2_void_Edit_Existing_Patient(Patient *PointerToPatientsData)
{
	...
	if (First_Choice == L_u8_Choosen_option)
	{
		...
		PointerToPatientsData[L_s32_iteration].L_u32_ID=L_u32_Entered_ID;
...
void AdmOpt3_void_Reserve_A_Slot(Slots*PointerToSlots,Patient *PointerToPatientsData)
{
	...
	if (First_Choice == L_u8_Choosen_option)
	{
		...
		PointerToSlots[First_Choice-1].L_u32_Patient_ID = L_u32_Entered_ID;
	}
	else if (Second_Choice == L_u8_Choosen_option)
	{
		PointerToSlots[Second_Choice-1].L_u8_avaliability_Of_Slot = Not_Available;
		PointerToSlots[Second_Choice-1].L_u32_Patient_ID = L_u32_Entered_ID;
	}
	else if (Third_Choice == L_u8_Choosen_option)
	{
		PointerToSlots[Third_Choice-1].L_u8_avaliability_Of_Slot = Not_Available;
		PointerToSlots[Third_Choice-1].L_u32_Patient_ID = L_u32_Entered_ID;
	}
	else if (Fourth_Choice == L_u8_Choosen_option)
	{
		PointerToSlots[Fourth_Choice-1].L_u8_avaliability_Of_Slot = Not_Available;
		PointerToSlots[Fourth_Choice-1].L_u32_Patient_ID = L_u32_Entered_ID;
	}
	else if (Fifth_Choice == L_u8_Choosen_option)
	{
		PointerToSlots[Fifth_Choice-1].L_u8_avaliability_Of_Slot = Not_Available;
		PointerToSlots[Fifth_Choice-1].L_u32_Patient_ID = L_u32_Entered_ID;
	}
	else
	{
		printf("wrong Selection!\n");
...
void AdmOpt4_void_Cancel_Reservation(Slots *PointerToSlots,Patient *PointerToPatientsData)
{
	...
	for(L_s32_iteration = IterationStart ; L_s32_iteration < AvailableSlots ; L_s32_iteration++)
	{
		if (PointerToSlots[L_s32_iteration].L_u32_Patient_ID == L_u32_Entered_ID)
		{
			PointerToSlots[L_s32_iteration].L_u8_avaliability_Of_Slot = Available;
			PointerToSlots[L_s32_iteration].L_u32_Patient_ID = 0;
...
void UsrOp1_void_View_Record(Patient* PointerToPatientsData)
{
	...
	for (L_s32_iteration = IterationStart; L_s32_iteration < NumberOfPatients ; L_s32_iteration++)
	{
		if(L_u32_Entered_ID == PointerToPatientsData[L_s32_iteration].L_u32_ID)
		{
			...
			printf("The Id is %d \n",PointerToPatientsData[L_s32_iteration].L_u32_ID);
		...
		else
		{
			...
			printf("1- Yes\n");
			printf("2- No\n");
			if(L_u8_Choosen_option== 1)
			UsrOp1_void_View_Record(PointerToPatientsData);
			else if(L_u8_Choosen_option== 2)
				void_User_Mode();
		}
	}
	...
	for(L_s32_iteration = IterationStart ; L_s32_iteration < AvailableSlots ; L_s32_iteration++)
		{
			...
			{
				printf("%d- %s and the id is %d\n",L_s32_iteration+1,PointerToSlots[L_s32_iteration].R_s8_time,PointerToSlots[L_s32_iteration].L_u32_Patient_ID);
			}
			else
			{
